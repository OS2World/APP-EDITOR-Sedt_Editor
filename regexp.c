/* ALTERED by Anker Berg-Sonne, Third argument to
 ...
 */
int Fast regexec(regexp *prog,char *string,int No_Bline)
{
	register char *s;
 ...
 */
int Fast regtry(regexp *prog,char *string)
{
	register int i;
	register char **sp;
	register char **ep;
...
int Fast regmatch(char *prog)
{
	...
	while (scan != NULL) {
		...
		case EXACTLY: {
			...
			break;
		case ANYOF:
			if (*reginput == '\0' || strchr(OPERAND(scan),
                         *reginput) == NULL)
				return(0);
			reginput++;
			break;
